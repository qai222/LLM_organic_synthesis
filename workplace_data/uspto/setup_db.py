"""
THIS IS FOR SETUP LOCAL SQL
DO NOT USE THIS IF YOU HAVE ALREADY DOWNLOADED PB.GZ FILES
...
def ord_add_data(dataset_id: str, local_data_folder: str = LOCAL_DATA_FOLDER, uspto_only=True):
    if local_data_folder:
        ...
        dataset = load_message(p, dataset_pb2.Dataset)
    else:
...
def get_dataset_ids():
    assert os.path.isfile("setup_db_dataset_ids_20230328.txt")
...
def get_existing_dataset_ids():
    ...
    query = session.query(_MESSAGE_TO_MAPPER[dataset_pb2.Dataset])
